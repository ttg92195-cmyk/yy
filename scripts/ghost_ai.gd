...
## GhostAI - ULTRA LIGHTWEIGHT - NO NavigationAgent3D
## Uses direct movement to targets instead of pathfinding
## This prevents the crash caused by NavigationMesh baking
...
func _physics_process(delta):
	...
	if head_mesh: head_mesh.position.y = 1.6 + fo
	if body_mesh: body_mesh.position.y = 0.8 + fo * 0.5
	...
	# Direct movement toward target (NO NavigationAgent3D)
...
func _set_state(s: GhostState):
	...
	if s == GhostState.HUNT and eye_light:
		eye_light.light_color = Color(1, 0.5, 0); eye_light.omni_range = 8.0
	elif s == GhostState.CHASE and eye_light:
		eye_light.light_color = Color(1, 0, 0); eye_light.omni_range = 10.0
	elif s == GhostState.PATROL and eye_light:
		eye_light.light_color = Color(1, 0.2, 0); eye_light.omni_range = 5.0


func _process_idle():
	velocity.x = 0; velocity.z = 0; move_target = Vector3.ZERO
	if state_timer > 3.0: _set_state(GhostState.PATROL)
	if hunt_cooldown <= 0 and randf() < 0.01: _start_hunt()


func _process_patrol():
	if patrol_points.is_empty(): _set_state(GhostState.IDLE); return
	...
	if global_position.distance_to(patrol_points[current_patrol_index]) < 2.0:
		...
		if current_patrol_index >= patrol_points.size() or current_patrol_index < 0:
			patrol_direction *= -1
			current_patrol_index += patrol_direction
	if hunt_cooldown <= 0 and randf() < 0.005: _start_hunt()
...
func _process_chase():
	if not target_player or not is_instance_valid(target_player):
		target_player = null; _set_state(GhostState.PATROL); return
	...
	if dist > lose_sight_range:
		...
		target_player = null; _set_state(GhostState.HUNT); return
	if dist < catch_distance: _set_state(GhostState.ATTACK)
