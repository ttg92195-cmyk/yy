...
func _setup_environment():
        ## Setup the horror world environment
        var world_env = WorldEnvironment.new()
        world_env.name = "WorldEnvironment"

        var env = Environment.new()
        env.background_mode = Environment.BG_COLOR
        env.background_color = Color(0.01, 0.01, 0.02, 1)
        env.ambient_light_source = Environment.AMBIENT_SOURCE_COLOR
        env.ambient_light_color = Color(0.08, 0.07, 0.1, 1)
        env.ambient_light_energy = 0.8

        # Fog for horror atmosphere (lighter for mobile visibility)
        env.fog_enabled = true
        env.fog_light_color = Color(0.03, 0.03, 0.05, 1)
        env.fog_light_energy = 0.5
        env.fog_density = 0.02
        env.fog_depth_begin = 10.0
        env.fog_depth_end = 40.0

        # Glow for flashlight effects
        env.glow_enabled = true
        env.glow_intensity = 0.6
        env.glow_strength = 0.7
        env.glow_bloom = 0.2

        # Tone mapping for better visibility
        ...
        dir_light.light_color = Color(0.3, 0.3, 0.5)
