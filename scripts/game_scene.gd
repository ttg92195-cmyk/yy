...
func _setup_environment():
	...
	# Ambient light - bright enough for mobile but still dark/spooky
	env.ambient_light_source = Environment.AMBIENT_SOURCE_COLOR
	env.ambient_light_color = Color(0.18, 0.15, 0.20, 1)
	env.ambient_light_energy = 2.0

	# Fog for horror atmosphere
	env.fog_enabled = true
	env.fog_light_color = Color(0.04, 0.04, 0.06, 1)
	...
	dir.light_color = Color(0.3, 0.3, 0.5)
