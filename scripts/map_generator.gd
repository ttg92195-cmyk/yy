...
func _setup_materials():
	...
	if floor_tex:
		...
		floor_mat.albedo_color = Color(0.25, 0.22, 0.20)
	else:
		floor_mat.albedo_color = Color(0.18, 0.16, 0.15)
	floor_mat.roughness = 0.95
	floor_mat.uv1_scale = Vector3(4, 1, 4)

	# Corridor floor - slightly different tile
	tile_floor_mat = StandardMaterial3D.new()
	tile_floor_mat.albedo_color = Color(0.15, 0.14, 0.13)
	...
	if wall_tex:
		...
		wall_mat.albedo_color = Color(0.22, 0.20, 0.19)
	wall_mat.roughness = 0.9
	wall_mat.uv1_scale = Vector3(2, 1.5, 2)

	# Dark wall for scary rooms
	dark_wall_mat = StandardMaterial3D.new()
	dark_wall_mat.albedo_color = Color(0.12, 0.10, 0.11)
	dark_wall_mat.roughness = 0.95

	# Ceiling
	ceiling_mat = StandardMaterial3D.new()
	ceiling_mat.albedo_color = Color(0.10, 0.09, 0.10)
...
func _generate_map():
	...
	# === LIGHTS (5 total - minimal for mobile) ===
	_make_light(Vector3(0, 2.7, 0), Color(0.6, 0.5, 0.3), 1.2, 16.0)     # Main corridor center
	_make_light(Vector3(-7, 2.7, 0), Color(0.5, 0.4, 0.3), 0.8, 12.0)    # Main corridor west
	_make_light(Vector3(7, 2.7, 0), Color(0.5, 0.4, 0.3), 0.8, 12.0)     # Main corridor east
	_make_light(Vector3(0, 2.7, 7), Color(0.4, 0.35, 0.3), 0.6, 10.0)    # Side corridor
	_make_light(Vector3(0, 2.7, 11.5), Color(0.3, 0.15, 0.2), 0.5, 8.0)  # Morgue (dim red)
