...
## Creates a proper hospital layout with corridors, rooms, furniture, blood stains
...
func _setup_materials():
        # Floor - dark hospital tile (brighter for visibility)
        ...
        if floor_tex:
                ...
                floor_mat.albedo_color = Color(0.40, 0.35, 0.30)
        else:
                floor_mat.albedo_color = Color(0.30, 0.26, 0.22)
        floor_mat.roughness = 0.95
        floor_mat.uv1_scale = Vector3(4, 1, 4)

        # Corridor floor - lighter tile
        tile_floor_mat = StandardMaterial3D.new()
        tile_floor_mat.albedo_color = Color(0.28, 0.25, 0.22)
        tile_floor_mat.roughness = 0.85
        tile_floor_mat.uv1_scale = Vector3(3, 1, 3)

        # Wall - brick texture (brighter)
...
func _generate_map():
        ...
        _make_wall_segment_x(Vector3(-10, wall_height/2, -2), 3.0)  # Far left
        _make_door_gap_x(Vector3(-8, wall_height/2, -2))             # Room 1 door
        _make_wall_segment_x(Vector3(-5.5, wall_height/2, -2), 2.0) # Between doors
        _make_door_gap_x(Vector3(0, wall_height/2, -2))              # Room 2 door
        _make_wall_segment_x(Vector3(2.5, wall_height/2, -2), 2.0)  # Between doors
        _make_door_gap_x(Vector3(8, wall_height/2, -2))              # Room 3 door
        _make_wall_segment_x(Vector3(10.5, wall_height/2, -2), 3.0) # Far right

        # South corridor wall with 3 door gaps
        _make_wall_segment_x(Vector3(-10, wall_height/2, 2), 3.0)
        _make_door_gap_x(Vector3(-8, wall_height/2, 2))              # Room 4 door
        _make_wall_segment_x(Vector3(-5.5, wall_height/2, 2), 2.0)
        _make_door_gap_x(Vector3(0, wall_height/2, 2))               # Room 5 door
        _make_wall_segment_x(Vector3(2.5, wall_height/2, 2), 2.0)
        _make_door_gap_x(Vector3(8, wall_height/2, 2))               # Room 6 door
        ...
        # === LIGHTS (10 total - enough to see on mobile!) ===
        _make_light(Vector3(0, 2.7, 0), Color(0.8, 0.7, 0.5), 2.0, 20.0)      # Main corridor center (BRIGHT)
        _make_light(Vector3(-7, 2.7, 0), Color(0.7, 0.6, 0.4), 1.5, 16.0)     # Main corridor west
        _make_light(Vector3(7, 2.7, 0), Color(0.7, 0.6, 0.4), 1.5, 16.0)     # Main corridor east
        _make_light(Vector3(-10, 2.7, -5), Color(0.6, 0.5, 0.4), 1.2, 14.0)  # Surgery room
        _make_light(Vector3(0, 2.7, -5), Color(0.6, 0.55, 0.4), 1.2, 14.0)   # Patient room
        _make_light(Vector3(10, 2.7, -5), Color(0.6, 0.55, 0.4), 1.0, 14.0)  # Lab
        _make_light(Vector3(-10, 2.7, 5), Color(0.6, 0.5, 0.4), 1.0, 14.0)   # Pharmacy
        _make_light(Vector3(0, 2.7, 7), Color(0.6, 0.5, 0.4), 1.2, 14.0)     # Side corridor
        _make_light(Vector3(10, 2.7, 5), Color(0.6, 0.5, 0.4), 1.0, 14.0)    # Ward B
        _make_light(Vector3(0, 2.7, 11.5), Color(0.5, 0.2, 0.25), 0.8, 10.0) # Morgue (dim red)
...
func _make_door_gap_x(center: Vector3):
        ## Create door frame walls (gap in wall for door opening)
        # Left wall piece
...
func _make_door_gap_z(center: Vector3):
        ## Create door frame walls for Z-axis wall
...
func _add_room_labels():
        var room_labels = [
                {"pos": Vector3(-8, 2.2, -2.2), "text": "SURGERY", "color": Color(0.8, 0.3, 0.3)},
                {"pos": Vector3(0, 2.2, -2.2), "text": "PATIENT ROOM", "color": Color(0.5, 0.7, 0.5)},
                {"pos": Vector3(8, 2.2, -2.2), "text": "LAB", "color": Color(0.5, 0.5, 0.8)},
                {"pos": Vector3(-8, 2.2, 2.2), "text": "PHARMACY", "color": Color(0.6, 0.8, 0.3)},
                {"pos": Vector3(0, 2.2, 2.2), "text": "STORAGE", "color": Color(0.7, 0.6, 0.4)},
                {"pos": Vector3(8, 2.2, 2.2), "text": "WARD B", "color": Color(0.4, 0.7, 0.7)},
                {"pos": Vector3(2.3, 2.2, 5), "text": "OFFICE", "color": Color(0.7, 0.7, 0.5)},
                {"pos": Vector3(2.3, 2.2, 9), "text": "MORGUE", "color": Color(0.9, 0.2, 0.2)},
        ]
        ...
        exit_sign.modulate = Color(0.2, 0.8, 0.2)
        exit_sign.billboard = 1
        add_child(exit_sign)


func _add_furniture():
        # Room 1: Surgery (X: -12 to -4, Z: -8 to -2)
        _make_decoration(Vector3(-9, 0.4, -5), Vector3(2.5, 0.8, 1.2), furniture_metal_mat)   # Operating table
        _make_decoration(Vector3(-9, 0.85, -5), Vector3(2.3, 0.1, 1.0), bed_mat)              # Table pad
        _make_decoration(Vector3(-11, 0.5, -7), Vector3(0.8, 1.0, 0.5), furniture_metal_mat)  # Equipment cart
        _make_decoration(Vector3(-7, 0.8, -3.5), Vector3(0.05, 1.6, 0.05), furniture_metal_mat) # IV stand pole
        _make_decoration(Vector3(-7, 1.6, -3.5), Vector3(0.3, 0.05, 0.3), furniture_metal_mat)  # IV stand top

        # Room 2: Patient Room (X: -4 to 4, Z: -8 to -2)
        _make_decoration(Vector3(-2, 0.3, -5), Vector3(1.8, 0.6, 0.9), furniture_wood_mat)   # Bed 1
        _make_decoration(Vector3(-2, 0.6, -5), Vector3(1.6, 0.1, 0.7), bed_mat)              # Mattress 1
        _make_decoration(Vector3(2, 0.3, -5), Vector3(1.8, 0.6, 0.9), furniture_wood_mat)    # Bed 2
        _make_decoration(Vector3(2, 0.6, -5), Vector3(1.6, 0.1, 0.7), bed_mat)               # Mattress 2
        _make_decoration(Vector3(-0.5, 0.3, -3.5), Vector3(0.5, 0.6, 0.4), furniture_wood_mat) # Nightstand

        # Room 3: Lab (X: 4 to 12, Z: -8 to -2)
        _make_decoration(Vector3(7, 0.4, -5), Vector3(3.0, 0.8, 0.8), furniture_wood_mat)    # Lab bench
        _make_decoration(Vector3(10, 0.4, -6), Vector3(0.6, 0.8, 0.6), furniture_metal_mat)  # Microscope table
        _make_decoration(Vector3(9, 1.5, -7.8), Vector3(2.0, 0.1, 0.4), furniture_wood_mat)  # Shelf

        # Room 4: Pharmacy (X: -12 to -4, Z: 2 to 8)
        _make_decoration(Vector3(-8, 0.5, 5), Vector3(2.5, 1.0, 0.6), furniture_wood_mat)    # Counter
        _make_decoration(Vector3(-11, 1.2, 6), Vector3(0.4, 1.8, 0.3), furniture_wood_mat)   # Shelf unit
        _make_decoration(Vector3(-10, 1.2, 7), Vector3(0.4, 1.8, 0.3), furniture_wood_mat)   # Shelf unit

        # Room 5: Storage (X: -4 to 4, Z: 2 to 8)
        _make_decoration(Vector3(-2, 0.4, 5), Vector3(1.5, 0.8, 1.0), furniture_wood_mat)    # Box
        _make_decoration(Vector3(1, 0.3, 4), Vector3(0.8, 0.6, 0.8), furniture_wood_mat)     # Small box
        _make_decoration(Vector3(2, 0.4, 6), Vector3(1.2, 0.8, 0.6), furniture_wood_mat)     # Cabinet

        # Room 6: Ward B (X: 4 to 12, Z: 2 to 8)
        _make_decoration(Vector3(6, 0.3, 4), Vector3(1.8, 0.6, 0.9), furniture_wood_mat)     # Bed 1
        _make_decoration(Vector3(6, 0.6, 4), Vector3(1.6, 0.1, 0.7), bed_mat)                # Mattress
        _make_decoration(Vector3(9, 0.3, 4), Vector3(1.8, 0.6, 0.9), furniture_wood_mat)     # Bed 2
        _make_decoration(Vector3(9, 0.6, 4), Vector3(1.6, 0.1, 0.7), bed_mat)                # Mattress
        _make_decoration(Vector3(11, 0.3, 7), Vector3(0.7, 0.6, 0.7), furniture_metal_mat)   # Wheelchair

        # Office area (east of side corridor)
        _make_decoration(Vector3(5, 0.35, 6), Vector3(1.5, 0.05, 0.8), furniture_wood_mat)   # Desk top
        _make_decoration(Vector3(5, 0.35, 6), Vector3(1.5, 0.7, 0.05), furniture_wood_mat)   # Desk front
        _make_decoration(Vector3(5, 0.25, 7), Vector3(0.5, 0.5, 0.5), furniture_wood_mat)    # Chair

        # Morgue area (Z: 10 to 12.5)
        _make_decoration(Vector3(-1, 0.3, 11.5), Vector3(2.0, 0.5, 0.8), furniture_metal_mat) # Autopsy table
        _make_decoration(Vector3(1, 0.5, 11.5), Vector3(0.8, 1.0, 1.5), furniture_metal_mat)  # Drawer unit
        # Body on table (dark shape)
...
func _make_decoration(position: Vector3, size: Vector3, mat):
        ## Visual-only box (no collision) for furniture/decorations
        var mesh = MeshInstance3D.new()
        var box = BoxMesh.new()
        box.size = size
        mesh.mesh = box
        if mat is StandardMaterial3D:
                ...
                m.albedo_color = mat
                m.roughness = 0.9
                mesh.set_surface_override_material(m)
