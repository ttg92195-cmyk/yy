...
func _setup_materials():
        # Floor material - dark tiled floor
        floor_mat = StandardMaterial3D.new()
        floor_mat.albedo_color = Color(0.12, 0.1, 0.1)
        floor_mat.roughness = 0.9
        floor_mat.metallic = 0.0

        # Wall material - dark stained walls
        wall_mat = StandardMaterial3D.new()
        wall_mat.albedo_color = Color(0.15, 0.13, 0.14)
        wall_mat.roughness = 0.85
        wall_mat.metallic = 0.0

        # Ceiling material
        ceiling_mat = StandardMaterial3D.new()
        ceiling_mat.albedo_color = Color(0.08, 0.07, 0.08)
...
func _create_obstacle(position: Vector3, size: Vector3):
        ...
        obs_mat.albedo_color = Color(0.2, 0.18, 0.17)
...
func _place_items():
        ...
        for item in item_data:
                ...
                $Items.add_child(item_scene) if has_node("Items") else add_child(item_scene)


func _create_item_node(type: int, display_name: String) -> Node3D:
        ## Create an item node with mesh and collision
        var area = Area3D.new()
...
func _place_escape_door():
        ...
        door_mat.albedo_color = Color(0.3, 0.15, 0.05)
